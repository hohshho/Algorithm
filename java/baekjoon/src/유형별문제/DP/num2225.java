...
public class num2225 {
    ...
    static HashMap<StringBuilder, StringBuilder> switches = new HashMap<>();
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (!q.isEmpty()) {
            int[] cur = q.poll();
            int x = cur[0], y = cur[1];
            StringBuilder curXY = new StringBuilder();
            curXY.append(x).append(y);

            // 1. 불켜기
            if (switches.containsKey(curXY)) {
                StringBuilder rooms = switches.get(curXY);

                for (int j = 0; j < rooms.length(); j += 2) {
                    int roomX = rooms.charAt(j) - '0';
                    int roomY = rooms.charAt(j + 1) - '0';
